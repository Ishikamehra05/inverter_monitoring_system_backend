...
type PlantChartExportContext = {
	params: Promise<{
		plantId: string;
	}>;
};

async function getPlantChartExportRoute(
	request: NextRequest,
	context: PlantChartExportContext,
): Promise<Response> {
	...
	try {
		...
		return errorResponse('Failed to generate chart export', 500);
	}
}

export const GET = withRequestLogging(requireAuth(getPlantChartExportRoute), {
	routeName: 'monitor.plants.chart.export',
});
