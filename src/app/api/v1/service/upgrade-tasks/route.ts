...
async function resolveFirmware(
  ...
  | { ok: true; firmwareId: string; firmwareUrl: string }
...
> {
  if (firmwareId) {
    ...
    if (!row) {
      return {
        ok: false,
        message: "firmwareId does not reference an existing firmware record",
      };
    }
    if (row.version !== newFirmwareVersion || row.chip_type !== chipType) {
      return {
        ok: false,
        message: "firmwareId does not match newFirmwareVersion/chipType",
      };
    }
    return {
      ok: true,
      firmwareId: row.id,
      firmwareUrl: row.file_path.replace(/\\/g, "/"),
    };
  }
...
async function resolvePlaceholderPlantId(actorId: bigint): Promise<bigint> {
  return prisma.$transaction(async (tx) => {
    ...
    if (!ownerAccount) {
      throw new Error(
        "Could not resolve an owning account for the placeholder plant",
      );
    }
...
async function postUpgradeTask(request: NextRequest): Promise<Response> {
  ...
  if (!firmwareResolution.ok) {
	...
	const firmwareUrl = `http://fota.solarlogger.in/firware${firmwareResolution.firmwareUrl}`;
  ...
  if (data.devices.some((device) => device.plantId === undefined)) {
    try {
      ...
      return errorResponse(
        `Could not resolve a default plant for devices missing plantId: ${message}`,
        500,
      );
    }
  }
  ...
  try {
    resolvedDevices = data.devices.map((device) => {
      const loggerImei =
        device.loggerImei ?? imeiMap.get(device.inverterSerialNo);
  ...
  const uniquePlantIds = Array.from(
    new Set(resolvedDevices.map((device) => device.plantId)),
  );
  ...
  const missingPlantIds = uniquePlantIds.filter(
    (id) => !foundPlantIds.has(id.toString()),
  );
  if (missingPlantIds.length > 0) {
    return errorResponse(
      `Plant(s) not found: ${missingPlantIds.join(", ")}`,
      404,
    );
  }
  ...
  if (conflictRows.length > 0) {
    return new Response(
      JSON.stringify({
        success: false,
        message:
          "One or more devices already have an active FOTA job; no jobs were created for this task",
        data: {
...
async function getUpgradeTasks(request: NextRequest): Promise<Response> {
  ...
  const nameFilterSql = name
    ? Prisma.sql`
      WHERE ut."deletedAt" IS NULL
        AND ut.name ILIKE ${`%${name}%`}
    `
    : Prisma.sql`
      WHERE ut."deletedAt" IS NULL
    `;
