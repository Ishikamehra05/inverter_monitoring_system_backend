...
async function getUpgradeTaskDetail(
  ...
  const aggregateStatus =
    failedJobs > 0
      ? "Failed"
      : jobRows.length > 0 && completedJobs === jobRows.length
        ? "Finished"
        : "In Progress";
...
async function deleteUpgradeTask(
  _request: NextRequest,
  context: RouteContext,
): Promise<Response> {
  const { taskId } = await context.params;
...
export const DELETE = withRequestLogging(requireAuth(deleteUpgradeTask), {
  routeName: "service.upgrade_tasks.delete",
});
