...
export class PlantRepository {
  ...
  private buildChartValue(
    ...
    const value =
      seed * rangeMultiplier +
      bucketIndex * (range === "day" ? 0.5 : range === "month" ? 3 : 12);
  ...
  async getPlantOverview(params: PlantOverviewParams) {
    ...
    if (serialNumbers.length) {
      ...
      const latestPerInverter = await prisma.deviceLogsLatest.groupBy({
        by: ["sno"],
    ...
    return {
      ...
      lastUpdatedAt: this.formatDateTime(
        aggregates.latestTimestamp ?? plant.lastUpdatedAt,
      ),
    };
  }
  ...
  async getPlantOverviewLive(params: PlantOverviewParams) {
    ...
    const aggregates = await prisma.deviceLogsLatest.aggregate({
      where: {
        sno: {
          in: serialNumbers,
        },
    ...
    // Latest timestamp
    const latestLog = await prisma.deviceLogsLatest.findFirst({
      where: {
        sno: {
          in: serialNumbers,
        },
    ...
    const totalEnergyValue =
      totalETotal >= 1000 ? totalETotal / 1000 : totalETotal;
    ...
    return {
      ...
      lastUpdatedAt: latestLog?.latestTimestamp
        ? this.formatDateTime(latestLog.latestTimestamp)
        : null,
    };
  }
  ...
  async getPlantAnalysisParameters(params: PlantAnalysisParametersParams) {
    ...
    const groups = Array.from(new Set(catalog.map((item) => item.group))).map(
      (group) => ({
        label: group,
        parameters: catalog
  ...
  async getPlantChart(params: PlantChartParams) {
    ...
    if (params.range === "day") {
      ...
      points = logs.map((log) => {
        ...
        const loggerIndex = devices.findIndex(
          (device) => device.serialNumber === log.sno,
        );

        point[`inverter${loggerIndex + 1}`] = Number(
          log.total_input_power ?? 0,
        );
      ...
      points = Array.from(
        ...
        (_, index) => {
          ...
          const dayLogs = latestLogs.filter(
            (log) => new Date(log.dayDate).getDate() === day,
          );
      ...
      points = months.map((month, monthIndex) => {
        ...
        const monthLogs = latestLogs.filter((log) => {
          const istDate = new Date(
            new Date(log.dayDate).toLocaleString("en-US", {
  ...
  async getPlantAnalysis(params: PlantAnalysisParams) {
    ...
    const selectedParameters = params.parameters
      .map((key) => key.trim())
      .filter(
        (key, index, array) => key.length > 0 && array.indexOf(key) === index,
      )
      .map((key) => catalog.find((item) => item.key === key))
      .filter((item): item is (typeof catalog)[number] => Boolean(item))
      .map((item) => ({
  ...
  async listPlantInverterSerials(plantId: bigint) {
  ...
  async getLatestTelemetryBySerials(serialNumbers: string[]) {
    const rows = await prisma.deviceLogsLatest.findMany({
      where: {
        sno: {
          in: serialNumbers,
        },
  ...
  async findPlantInformationById(plantId: string) {
  ...
  async getPlantList(params: PlantListParams) {
    ...
    try {
      ...
      const plantSerialMap = new Map<string, string[]>();
      ...
      const logs = await prisma.deviceLogsLatest.findMany({
        where: {
          sno: {
            in: serials,
          },
      ...
      let items = plants.map((plant) => {
        ...
        for (const serial of serials) {
          ...
          if (
            log.latestTimestamp &&
            (!latestUpdate || log.latestTimestamp > latestUpdate)
          ) {
            latestUpdate = log.latestTimestamp;
          }
        }
        const effect =
          plant.kwp && plant.kwp > 0
            ? Number((eToday / plant.kwp).toFixed(2))
            : 0;
        return {
          id: String(plant.id),
          ...
          pictureUrl: plant.pictureFileId
            ? `https://napi.solarlogger.in/api/v1/uploads/plants/${encodeURIComponent(
          ...
          eTotal: {
            value: (eTotal >= 1000 ? eTotal / 1000 : eTotal).toFixed(2),
      ...
      for (const plant of plants) {
        ...
        switch (current.status) {
          ...
          case PlantStatus.Abnormal:
            statusCounts.Abnormal++;
            break;

          case PlantStatus.Standby:
    ...
    try {
      ...
      if (!serialNumbers.length) {
        return {
          ...
          capacity: {
            value: Number(
              plants.reduce((sum, p) => sum + (p.kwp ?? 0), 0).toFixed(2),
            ),
            unit: "kW",
          },
      ...
      if (latestConditions.length) {
        latestLogs = await prisma.deviceLogsLatest.findMany({
          where: {
            OR: latestConditions,
          },
      ...
      return {
        ...
        eTotal: {
          value: Number(
            (totalEnergy >= 1000 ? totalEnergy / 1000 : totalEnergy).toFixed(2),
          ),
    ...
    try {
      const existingPlant = await prisma.plant.findFirst({
        where: {
          userAccount: plantOwnerAccount,
          name: {
            equals: plantData.plantName.trim(),
          },
        },
      });

      if (existingPlant) {
        throw new ApiError(409, "Plant name already exists");
      }
      const newPlant = await prisma.plant.create({
        data: {
          name: plantData.plantName,
